...
class ControlUnit:
   ...
   def dispatch_micro_instruction(self, microcode: list):
      ...
      for signal in microcode:
         if signal in [MUX.MPC_INC, MUX.MPC_OPCODE, MUX.MPC_ZERO]:
         ...
         elif signal in [MUX.PC_ADDR, MUX.PC_INC]:
            ...
            match (signal):
               ...
               case IO.OUT:
                  self.datapath.signal_output(self.datapath.return_stack[self.datapath.return_stack_pointer], self.datapath.stack[self.datapath.stack_pointer])
               case MEMORY.RD:
                  self.datapath.signal_memory_read(self.datapath.stack[self.datapath.stack_pointer])
               case MEMORY.WR:
                  self.datapath.signal_memory_write(self.datapath.stack[self.datapath.stack_pointer-1], self.datapath.stack[self.datapath.stack_pointer])
               case _:
                  pass
      self.tick()

   def __repr__(self):
      state = (f"[{self.datapath._pc}: {self._ir.opcode if self._ir is not None else 'NO_OPCODE' }] TICK: {self.current_tick()} MPC: {self.mpc} "
               f"IR: {self._ir} TOP: {self.datapath.stack[self.datapath.stack_pointer]} "
               f"NEXT: {self.datapath.stack[self.datapath.stack_pointer]} "
